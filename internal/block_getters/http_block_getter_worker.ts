...
import { HttpBlockGetter } from "./http_block_getter.js";
...
parentPort.on(
    "message",
    async (message: { blockNumber: number; callBackId: number }) => {
        try {
            parentPort?.postMessage({
                callBackId: message.callBackId,
                error: null,
                block: await blockGetter.getBlockWithTransactionReceipts(
                    message.blockNumber
                ),
