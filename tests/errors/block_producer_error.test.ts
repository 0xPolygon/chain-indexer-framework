...
describe("Block Producer Error", () => {
    let blockProducerError: BlockProducerError;
    ...
    beforeEach(() => {
        blockProducerError = new BlockProducerError();
    ...
    describe("Block producer error class", () => {
        test("Block producer error object by default must have 'Block producer error' as name set via super()",
            () => {
        ...
        test("Block producer error object by default must have 4000 as error code set via super()",
            () => {
                expect(mockedBaseErrorClass).toBeCalledWith(
                    expect.anything(),
                    4000,
        ...
        test("Block producer error class must have a static property codes which is an object of all block producer codes",
            () => {
                expect(BlockProducerError.codes).toEqual({
                    BASE_ERROR: 100,
                    UNKNOWN_ERR: 4000,
    ...
    describe("createUnknown", () => {
        test("static method must return error as is, if an instance of base error",
            () => {
                ...
                expect(BlockProducerError.createUnknown(blockProducerError)).toBe(blockProducerError);
            }
        );
        ...
        test("static method must convert any unknown error type to error object by calling createErrorObject",
            () => {
                ...
                expect(BlockProducerError.createUnknown("mock")).toBeInstanceOf(BlockProducerError);
        ...
        test("static method must set error message to be message of the created error object from unknown",
            () => {
                mockedIsBaseError.mockReturnValueOnce(false);
                mockedCreateErrorObject.mockReturnValueOnce(new Error("mock"));
                BlockProducerError.createUnknown("mock");
        ...
        test("static method must set origin to 'local' if isLocal is true",
            () => {
                mockedIsBaseError.mockReturnValueOnce(false);
                mockedCreateErrorObject.mockReturnValueOnce(new Error("mock"));
                BlockProducerError.createUnknown(new Error("mock"), true);
        ...
        test("static method must set origin to 'remote' if isLocal is false",
            () => {
                mockedIsBaseError.mockReturnValueOnce(false);
                mockedCreateErrorObject.mockReturnValueOnce(new Error("mock"));
                BlockProducerError.createUnknown(new Error("mock"), false);
